...
test_that("Invalid on invalid inputs", {
  ...
  expect_error(PDBCleanup::alignDomains(NA, NA, NA))
  expect_error(PDBCleanup::alignDomains(predicted6ofs, NA, domains6ofs))
  expect_error(PDBCleanup::alignDomains(NA, experimental6ofs, domains6ofs))
  expect_error(PDBCleanup::alignDomains(predicted6ofs, experimental6ofs, NA))
})
...
test_that("Reduced RMSD after alignment", {
  ...
  predictedAligned <- PDBCleanup::alignDomains(experimental6ofs, predicted6ofs,
                                               domains6ofs)
